...
# This is the code for multiple simulation of the Braitenberg Vehicle
# Edit lines 345 and 346 to save results
...
def run_sim(g_synpk_ex, g_synpk_in):
    map_size = 100
    
    global foodx, foody, predx, predy, food_count, pred_count, bug_plot, food_plot, pred_plot, sr_plot, sl_plot
    foodx = 50
    foody = 50
    predx = -50
    predy = -50
    ...
    g_synmaxval_ex=(g_synpk_in/(tau_ampa/ms*exp(-1)))
    g_synmaxval_in=(g_synpk_in/(tau_ampa/ms*exp(-1)))
    ...
    Iin = gin * (Ein -v) : 1
    dgin/dt = -gin/tau_ampa + zin/ms : 1
    dzin/dt = -zin/tau_ampa : 1
    ...
    return pred_count, food_count

g_in = [0, 0.2, 0.4, 0.6, 0.8, 1]
g_ex = g_in

pred_count_full = zeros(36)
food_count_full = zeros(36)

#for kk in range(11,11):
kk = 10
for ii in range(0,6):
    for jj in range(0,6):
        index_helper = ii*6 + jj
        pred_count_full[index_helper], food_count_full[index_helper] = run_sim(g_in[ii],g_ex[jj])

# EDIT LINES BELOW TO SAVE RESULTS
#pred_count_full.tofile(r"C:\Users\Bryan\Documents\BME503\Final Report\pred_count_full"+str(kk)+r".csv",sep=',',format='%10.5f')
#food_count_full.tofile(r"C:\Users\Bryan\Documents\BME503\Final Report\food_count_full"+str(kk)+r".csv",sep=',',format='%10.5f')